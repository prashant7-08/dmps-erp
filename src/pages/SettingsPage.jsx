...
export const SettingsPage = ({ initialTab = 'global' }) => {
  ...
  const handleDeleteRole = (roleToDelete) => {
    ...
    if (window.confirm(`Are you sure you want to delete the role "${roleToDelete}"?`)) {
      const updated = rolesList.filter(r => r !== roleToDelete);
      setRolesList(updated);
      try {
        localStorage.setItem('DMPS_CUSTOM_ROLES_V1', JSON.stringify(updated));
      } catch (e) {}
      if (selectedRole === roleToDelete) {
        setSelectedRole(updated[0] || 'Teacher');
      }
      showToast(`Role "${roleToDelete}" deleted successfully! 🗑️`, 'success');
    }
  };
  ...
  const handleAddRoleSubmit = (e) => {
    ...
    if (rolesList.some(r => r.toLowerCase() === trimmed.toLowerCase())) {
      showToast(`Role "${trimmed}" already exists!`, 'warning');
      return;
    }
    const updated = [...rolesList, trimmed];
    setRolesList(updated);
    ...
    showToast(`🎉 New role "${trimmed}" added successfully!`, 'success');
  };
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase text-slate-500 dark:text-slate-400 tracking-wider">
                System Roles ({rolesList.length} Active):
              </span>
              <span className="text-[11px] text-slate-400">
                Click a role to view/edit permissions • Click 🗑️ to delete a role
              </span>
            </div>
                    ...
                    {!isProtected && (
                      <button
                        ...
                        className={`p-1 rounded-lg transition-colors cursor-pointer ml-1 ${
                          isSelected ? 'hover:bg-rose-600 text-rose-200 hover:text-white' : 'hover:bg-rose-100 text-slate-400 hover:text-rose-600 dark:hover:bg-rose-950/60'
                        }`}
                      >
        ...
        <form onSubmit={handleAddRoleSubmit} className="space-y-4 text-xs">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Exam Coordinator, Bus In-Charge, Sports Head"
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            <button
              ...
              className="px-4 py-2 text-slate-500 font-bold rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
            >
              Cancel
            </button>
            <button
